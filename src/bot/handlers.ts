...
import {
  ...
  calculateNotificationTimeFromDateStr,
  utcToLocalTime,
  getTodayISO
  ...
  private async executeAction(userId: number, chatId: number, action: string, params: string[]): Promise<void> {
    ...
    switch (action) {
      ...
      case 'back_to_preview':
        await this.showPreview(userId, chatId);
        break;
      default:
...
_Формат: ДД/ММ/ГГГГ (например: ${today})_`,
  ...
  private async handleDateInput(userId: number, chatId: number, text: string, session: UserSession): Promise<void> {
    ...
    if (isDateStringInPast(result.dateStr, tz)) {
      await this.api.sendMessageWithKeyboard(
        chatId,
        `❌ Дата *${formatDateStr(result.dateStr, 'long')}* уже прошла.
...
📅 Введите будущую дату:`,
    ...
    this.saveSession(userId, chatId, {
      ...
      data: { ...session.data, temp_date: result.dateStr }
    });

    // ВРЕМЯ ОБЯЗАТЕЛЬНО - нет кнопки "Пропустить"
    await this.api.sendMessageWithKeyboard(
      chatId,
      `✅ Дата: *${formatDateStr(result.dateStr, 'long')}*
...
_Например: 14-30 или 09-00_`,
    ...
    if (dateStr) {
      ...
      if (isDateTimeInPast(dateStr, timeStr, tz)) {
...
Или пропустите, если описание не нужно:`,
  ...
  private async showPeriodMenu(userId: number, chatId: number): Promise<void> {
    ...
    const description = data?.temp_description || 'нет';
    
    let infoText = `📌 *Название:* ${title}`;
    if (data?.temp_date) {
      infoText += `\n📅 *Дата:* ${formatDateStr(data.temp_date, 'long')}`;
      if (data.temp_time) {
        infoText += ` в ${data.temp_time}`;
      }
    }
...
📅 *Дата:* ${formatDateStr(data.temp_date, 'long')} в ${data.temp_time}
...
Всё верно?`;
  ...
  // ==================== EDIT MENU (при создании) - ИСПРАВЛЕНО: добавлена кнопка "Назад" ====================
...
📌 *${data.temp_title}*`;
    ...
    const buttons: InlineKeyboardButton[][] = [
      ...
      [callbackButton('◀️ Назад к проверке', 'back_to_preview')],
      [callbackButton('❌ Отмена', 'cancel')]
    ];
...
_Текущее: ${session.data?.temp_title}_`,
  ...
  private async startEditDate(userId: number, chatId: number): Promise<void> {
    ...
    const currentDate = session.data?.temp_date 
      ? formatDateStr(session.data.temp_date, 'long')
      : 'не указана';
    const today = this.getTodayString(tz);
    await this.api.sendMessageWithKeyboard(
      chatId,
...
📌 *${reminder.title}*
📅 Дата: ${formatDateStr(reminder.event_date, 'long')} в ${reminder.event_time}
...
🔄 Повтор: ${repeatLabel}`;
  ...
  private async showRemindersList(userId: number, chatId: number): Promise<void> {
    ...
    reminders.slice(0, 10).forEach((r, i) => {
      text += `${i + 1}. ${r.title}\n   📅 ${formatDateStr(r.event_date, 'long')} в ${r.event_time}\n\n`;
    });
  ...
  // ==================== REMINDER DETAILS - ИСПРАВЛЕНО: разные кнопки для активных и архивных ====================
...
📅 *Дата:* ${formatDateStr(r.event_date, 'long')} в ${r.event_time}
...
📆 *Создано:* ${createdStr}`;
    ...
    // ИСПРАВЛЕНО: Разные кнопки для активных и архивных напоминаний
    let buttons: InlineKeyboardButton[][];
    
    if (r.is_active) {
      // Для АКТИВНЫХ напоминаний
      buttons = [
      ...
      // Для АРХИВНЫХ напоминаний
      buttons = [
        [callbackButton('↩️ Восстановить', `restore_reminder:${id}`)],
        [callbackButton('🗑 Удалить навсегда', `delete_reminder:${id}`)],
        [callbackButton('◀️ К архиву', 'archived_reminders')]
      ];
    }
...
❌ Это действие нельзя отменить!`,
  ...
  private async showEditExistingMenu(userId: number, chatId: number, id: string): Promise<void> {
    ...
    const periodLabel = formatPeriod(r.reminder_periods?.[0] || 86400000);
    const repeatLabel = this.getRepeatLabel(r.repeat_type, r.repeat_days, r.repeat_month_day);
...
📌 *${r.title}*
📅 ${formatDateStr(r.event_date, 'long')} в ${r.event_time}
📝 ${r.description || 'нет описания'}
🔔 ${periodLabel}
🔄 ${repeatLabel}`;

    // РАСШИРЕННОЕ МЕНЮ с Периодом и Повтором + кнопка Назад
    const buttons: InlineKeyboardButton[][] = [
...
_Текущее: ${r.title}_`,
  ...
  private async startEditExistingDate(userId: number, chatId: number, id: string): Promise<void> {
    ...
    const currentDate = r.event_date ? formatDateStr(r.event_date, 'long') : 'не указана';
    await this.api.sendMessageWithKeyboard(
      chatId,
      `📅 Введите новую дату:
_Текущая: ${currentDate}_
...
📌 ${data.temp_title}
📅 ${formatDateStr(data.temp_date, 'long')} в ${data.temp_time}
