...
export class PamPinBot {
  ...
  async processUpdate(update: Update): Promise<void> {
    ...
    try {
      if (update.update_type === 'message_created' && update.message) {
        ...
        if (chatId) await this.sendMenu(chatId);
      }
    } catch (e) {
  ...
  private async send(chatId: number, text: string, buttons?: InlineKeyboardButton[][]): Promise<void> {
    console.log(`[Bot] Send to ${chatId}: ${text.substring(0, 30)}...`);
    try {
      if (buttons?.length) {
        await this.api.sendMessageWithKeyboard(chatId, text, buttons, 'plain');
      } else {
        await this.api.sendText(chatId, text, 'plain');
      }
  ...
  private async onMessage(msg: Message): Promise<void> {
    ...
    console.log(`[Bot] Msg: uid=${uid} cid=${cid} txt="${txt?.substring(0, 20)}"`);

    if (!txt || !uid || !cid) return;

    if (txt === '/start') { await this.sendMenu(cid); return; }
    if (txt === '/list') { await this.showList(uid, cid); return; }
    if (txt === '/add') { await this.startAdd(uid, cid); return; }
  ...
  private async onCallback(cb: MessageCallback): Promise<void> {
    ...
    if (pl === 'add') await this.startAdd(uid || 0, cid);
    else if (pl === 'list') await this.showList(uid || 0, cid);
    else if (pl === 'cancel') { clearUserSession(uid || 0, cid); await this.sendMenu(cid); }
    else if (pl === 'confirm') await this.doCreate(uid || 0, cid);
    else if (pl.startsWith('del:')) { deleteReminder(pl.split(':')[1]); await this.send(cid, '✅ Удалено'); }
  }
