...
🔄 Повтор: ${repeatLabel}`;
  ...
  private async showRemindersList(userId: number, chatId: number): Promise<void> {
    ...
    reminders.forEach((r, idx) => {
      ...
      const date = this.parseISODate(r.event_date, tz);
      const dateStr = this.formatDateForUser(date, tz);
      text += `${idx + 1}. *${r.title}*\n   📅 ${dateStr} в ${r.event_time}\n\n`;
    });

    const buttons: InlineKeyboardButton[][] = reminders.map((r) => [
      callbackButton(`📌 ${r.title.substring(0, 25)}${r.title.length > 25 ? '...' : ''}`, `view_reminder:${r.id}`)
    ]);
    buttons.push([callbackButton('🏠 Главное меню', 'main_menu')]);
  ...
  private async showReminderDetails(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[DETAILS] userId:', userId, 'reminderId:', reminderId);
    ...
    const tz = reminder.timezone || 'Europe/Moscow';
    const date = this.parseISODate(reminder.event_date, tz);
    const period = reminder.reminder_periods?.[0] || 86400000;
    const notificationTime = this.calculateNotificationTimeDisplay(reminder.event_date, reminder.event_time, period, tz);
    const periodLabel = formatPeriod(period);
    const repeatLabel = this.getRepeatLabel(reminder.repeat_type, reminder.repeat_days, reminder.repeat_month_day);
...
📅 *Дата:* ${this.formatDateForUser(date, tz)} в ${reminder.event_time}
📝 *Описание:* ${reminder.description || 'нет'}
🔔 *Напомнить:* ${periodLabel}
⏱ *Время уведомления:* ${notificationTime}
🔄 *Повтор:* ${repeatLabel}`;
    ...
    const buttons: InlineKeyboardButton[][] = [
      [
        callbackButton('✏️ Изменить', `edit_existing_reminder:${reminder.id}`),
        callbackButton('📦 В архив', `archive_reminder:${reminder.id}`)
      ],
      [callbackButton('🔔 Тест уведомления', `test_notification:${reminder.id}`)],
      [callbackButton('◀️ К списку', 'list_reminders')]
    ];
  ...
  private async showArchivedList(userId: number, chatId: number): Promise<void> {
    console.log('[ARCHIVED_LIST] userId:', userId, 'chatId:', chatId);
    const reminders = getArchivedRemindersByUser(userId, chatId);
    console.log('[ARCHIVED_LIST] Found:', reminders.length);

    if (reminders.length === 0) {
      await this.api.sendMessageWithKeyboard(
        chatId,
        '📦 *Архив пуст.*\n\nЗдесь будут храниться выполненные и удалённые напоминания.',
        [
  ...
  private async confirmArchive(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[CONFIRM_ARCHIVE] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
...
Напоминание будет деактивировано и перемещено в архив.`;

    const buttons: InlineKeyboardButton[][] = [
      [
        callbackButton('✅ Да, в архив', `confirm_archive:${reminderId}`),
  ...
  private async doArchive(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[DO_ARCHIVE] reminderId:', reminderId);
  ...
  private async confirmRestore(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[CONFIRM_RESTORE] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    const text = `♻️ *Восстановить напоминание?*

📌 ${reminder.title}`;

    const buttons: InlineKeyboardButton[][] = [
      [
        callbackButton('✅ Да, восстановить', `confirm_restore:${reminderId}`),
        callbackButton('❌ Отмена', `view_reminder:${reminderId}`)
      ]
    ];
  ...
  private async doRestore(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[DO_RESTORE] reminderId:', reminderId);
    restoreReminder(reminderId);
    await this.api.sendMessageWithKeyboard(
      chatId,
      '♻️ Напоминание восстановлено.',
      [
        [callbackButton('📋 Мои напоминания', 'list_reminders')],
        [callbackButton('📦 Архив', 'archived_reminders')]
      ],
  ...
  private async confirmDelete(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[CONFIRM_DELETE] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    const text = `🗑️ *Удалить напоминание навсегда?*
...
⚠️ Это действие нельзя отменить!`;
    ...
    const buttons: InlineKeyboardButton[][] = [
      [
        callbackButton('🗑️ Удалить', `confirm_delete:${reminderId}`),
        callbackButton('❌ Отмена', `cancel_delete:${reminderId}`)
      ]
    ];
  ...
  private async doDelete(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[DO_DELETE] reminderId:', reminderId);
    deleteReminderPermanently(reminderId);
    await this.api.sendMessageWithKeyboard(
      chatId,
      '🗑️ Напоминание удалено навсегда.',
      [
        [callbackButton('📋 Мои напоминания', 'list_reminders')],
        [callbackButton('📦 Архив', 'archived_reminders')]
      ],
  ...
  private async sendTestNotification(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[TEST_NOTIFY] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
...
📌 ${reminder.title}
📅 ${reminder.event_date} в ${reminder.event_time}
⏰ Уведомление придёт: ${notificationTime}
...
_Сейчас: ${nowStr} (${tz})_`;
  ...
  private async showEditExistingMenu(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_MENU] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    const text = `✏️ *Редактирование напоминания*
...
Что хотите изменить?`;
    ...
    const buttons: InlineKeyboardButton[][] = [
      [callbackButton('📝 Название', `edit_existing_title:${reminderId}`)],
      [callbackButton('📅 Дату', `edit_existing_date:${reminderId}`)],
      [callbackButton('🕐 Время', `edit_existing_time:${reminderId}`)],
      [callbackButton('📝 Описание', `edit_existing_description:${reminderId}`)],
      [callbackButton('🔔 Период', `edit_existing_period:${reminderId}`)],
      [callbackButton('🔄 Повтор', `edit_existing_repeat:${reminderId}`)],
      [callbackButton('🗑️ Удалить', `delete_reminder:${reminderId}`)],
      [callbackButton('◀️ Назад', `view_reminder:${reminderId}`)]
    ];
  ...
  private async startEditExistingTitle(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_TITLE] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    this.saveSession(userId, chatId, {
      state: 'editing_title',
      data: {
        editing_reminder_id: reminderId,
        temp_title: reminder.title,
        temp_description: reminder.description,
        temp_date: reminder.event_date,
        temp_time: reminder.event_time,
    ...
    await this.api.sendMessageWithKeyboard(
      chatId,
      `📝 Введите новое название:
_Текущее: ${reminder.title}_`,
      [[callbackButton('◀️ Отмена', `edit_existing_reminder:${reminderId}`)]],
      'markdown'
    );
  }

  private async startEditExistingDate(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_DATE] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    await this.api.sendMessageWithKeyboard(
      chatId,
      `📅 Введите новую дату:
_Текущая: ${this.formatDateForUser(date, tz)}_
_Формат: ДД/ММ/ГГГГ (например: ${today})_`,
      [[callbackButton('◀️ Отмена', `edit_existing_reminder:${reminderId}`)]],
      'markdown'
    );
  ...
  private async startEditExistingTime(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_TIME] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    await this.api.sendMessageWithKeyboard(
      chatId,
      `🕐 Введите новое время:
_Текущее: ${reminder.event_time}_
_Формат: ЧЧ-ММ (например: 14-30)_`,
      [[callbackButton('◀️ Отмена', `edit_existing_reminder:${reminderId}`)]],
      'markdown'
    );
  ...
  private async startEditExistingDescription(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_DESC] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
    ...
    await this.api.sendMessageWithKeyboard(
      chatId,
      `📝 Введите новое описание:
_Текущее: ${reminder.description || 'нет'}_
Или "нет" чтобы убрать описание:`,
      [[callbackButton('◀️ Отмена', `edit_existing_reminder:${reminderId}`)]],
      'markdown'
    );
  ...
  private async startEditExistingPeriod(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_PERIOD] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
  ...
  private async startEditExistingRepeat(userId: number, chatId: number, reminderId: string): Promise<void> {
    console.log('[EDIT_EXISTING_REPEAT] reminderId:', reminderId);
    const reminder = getReminderById(reminderId);
  ...
  private async saveEditedReminder(userId: number, chatId: number): Promise<void> {
    ...
    if (!data?.editing_reminder_id) {
      console.log('[SAVE_EDIT] No editing_reminder_id');
...
🕐 *Текущее время:* ${timeStr}`;
  ...
  private async doSetTimezone(userId: number, chatId: number, timezone: string): Promise<void> {
    ...
    // ИСПРАВЛЕНО: передаём один объект вместо трёх аргументов
...
5. Выберите период напоминания
6. Выберите тип повтора

*Форматы:*
📅 Дата: ДД/ММ/ГГГГ или ДД/ММ
🕐 Время: ЧЧ-ММ или ЧЧ:ММ

_Время указывается в вашем часовом поясе._`;
